...
async function processInput(inputBuffer) {
    ...
    try {
        ...
        switch (action) {
            ...
            case 'login':
                logmsg.login = await login(data, db);
                logmsg.get_user_profile = await get_user_profile(data, db);
                logmsg.get_chats = await get_chats(data, db);
                logmsg.get_active_sessions = await get_active_sessions(data, db);
                logmsg.get_online_users = await get_online_users(data, db);
                logmsg.get_deliver_messages = await get_deliver_messages(data, db);
                logmsg.get_deliver_sessions = await get_deliver_sessions(data, db);
                logmsg.get_my_sessions = await get_my_sessions(data, db);
                break;
            case 'get_chats':
                logmsg.get_chats = await get_chats(data, db);
                break;
            case 'get_users':
                logmsg.get_users = await get_users(data, db);
                break;
            case 'create_group':
                logmsg.create_group = await create_group(data, db);
                data.roomid = logmsg.create_group.create_group[0]?.RoomId || '';
                logmsg.get_receiver_sessions = await get_receiver_sessions(data, db);
                break;
            case 'send_message':
                logmsg.send_message = await send_message(data, db);
                logmsg.get_receiver_sessions = await get_receiver_sessions(data, db);
                break;
            case 'get_messages':
                logmsg.get_messages = await get_messages(data, db);
                logmsg.get_message_files = await get_message_files(data, db);
                logmsg.get_receiver_profile = await get_receiver_profile(data, db);
                logmsg.get_sender_messages = await get_sender_messages(data, db);
                logmsg.get_sender_sessions = await get_sender_sessions(data, db);
                break;
            case 'get_groups':
                logmsg.get_groups = await get_groups(data, db);
                break;
            case 'get_group_users':
                logmsg.get_group_users = await get_group_users(data, db);
                break;
            ...
            case 'get_common_groups':
                logmsg.get_common_groups = await get_common_groups(data, db);
                break;
            ...
            case 'get_receiver_profile':
                ...
                break;
            case 'chunk_upload':
                logmsg.chunk_upload = await chunk_upload(data, db, chunkData);
                break;
            case 'disconn':
                logmsg.disconn = await disconn(data, db);
                logmsg.get_active_sessions = await get_active_sessions(data, db);
                logmsg.get_online_users = await get_online_users(data, db);
                logmsg.get_my_sessions = await get_my_sessions(data, db);
                break;
            case 'chunk_download':
                const [tempLog, tempChunk] = await chunk_download(data, db);
                logmsg.chunk_download = tempLog;
                chunkDown = tempChunk;
                break;
            case 'get_max_chunkindex':
                logmsg.get_max_chunkindex = await get_max_chunkindex(data, db);
                break;
            case 'reset_status':
                logmsg.reset_status = await reset_status(data, db);
                break;
            case 'chunk_assemble':
                logmsg.chunk_assemble = await chunk_assemble(data, db);
                break;
            case 'file_download':
                return await file_download(data, db); // Special return for file download
            case 'get_message_files':
                logmsg.get_message_files = await get_message_files(data, db);
                break;
            case 'chunk_append':
                logmsg.chunk_append = await chunk_append(data, db);
                break;
            ...
            case 'terminate_session':
                logmsg.terminate_session = await terminate_session(data, db);
                break;

            // case related to mail

            case 'signupoptions':
                logmsg.signupoptions = await mailapi.signupoptions(data, db);
                break;
