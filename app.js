...
class EmailClient {
  ...
  bindEvents() {
    ...
    this.elements.addAccountBtn?.addEventListener('click', () => this.openEmailProviderModal());

    // Email provider modal
    this.elements.closeProviderModal?.addEventListener('click', () => this.closeEmailProviderModal());
  ...
  openEmailProviderModal() {
    this.closeUserDropdown();
    this.elements.emailProviderModal?.classList.add('active');
  ...
  closeEmailProviderModal() {
    this.elements.emailProviderModal?.classList.remove('active');
  }

  displayEmailProviders(providers) {
    ...
    this.elements.providerList.innerHTML = providers.map(provider => `
      <button class="provider-btn" data-provider-id="${provider.EmailSerProId}">
        <i class="fas fa-envelope"></i>
        <span>${provider.EmailSerProName}</span>
      </button>
    `).join('');
    ...
    document.querySelectorAll('.provider-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const providerName = e.currentTarget.querySelector('span').textContent;
        this.selectEmailProvider(providerId, providerName);
      });
    });
  }

  selectEmailProvider(providerId, providerName) {
    console.log('Selected provider:', providerId, providerName);
    this.closeEmailProviderModal();
    // this.showToast(`Selected ${providerName}`, 'success');
    this.openOAuthWindow(providerId);
  }
